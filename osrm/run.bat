...
docker run -t %mounts% ghcr.io/project-osrm/osrm-backend osrm-contract %where%.osrm

:: Runs the actual OSRM backend on port 5000.
docker run -t -i -p 5000:5000 -v "%curdir%/data:/data" ghcr.io/project-osrm/osrm-backend osrm-routed --algorithm ch %where%.osrm
