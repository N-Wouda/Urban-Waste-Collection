...
class GreedyStrategy:
    ...
    def plan(self, event: ShiftPlanEvent) -> list[Route]:
        ...
        model = self._make_model(container_idcs)
    ...
    def _make_model(self, container_idcs: np.ndarray[int]) -> Model:
        time_per_container = self.sim.config.TIME_PER_CONTAINER
        shift_duration = self.sim.config.SHIFT_DURATION

        model = Model()
        model.add_depot(
            x=f2i(self.sim.depot.location[0]),
            y=f2i(self.sim.depot.location[1]),
            tw_late=int(shift_duration.total_seconds()),
        )

        for container_idx in container_idcs:
            container = self.sim.containers[container_idx]
            model.add_client(
        ...
        vehicle_count = Counter(int(v.capacity) for v in self.sim.vehicles)
        ...
        distances = self.sim.distances
        durations = self.sim.durations / np.timedelta64(1, "s")
